...
- [ ] 并发编程
    ...
    5. Lock与Synchronized比较

        尝试非阻塞的获取锁（tryLock）、能被中断地获取锁、超时获取锁

    6. 重入锁（ReentrantLock）

        支持一个线程对资源可以重复加锁，已经在取到锁的线程，能够再次调用lock获取锁而不被阻塞
        公平性：等待时间最长的线程最优先获取锁，这个锁是公平锁

    7. 读写锁（ReentrantWriteReadLock）
    ...
    8. Condition
        ...
        |等待队列个数|一个|多个|
        |在等待状态不响应中断|不支持|支持awaitUninterruptibly|
        |当前线程释放锁并进入等待状态到将来的某个时间|不支持|支持awaitUtil|
        |唤醒线程|notify、notifyAll|signal、signalAll|
    ...
    14. 线程池
        ...
        合理使用线程池能够降低资源消耗，提高响应速度，提高线程的可管理性

        线程池常用的阻塞队列：

            * ArrayBlockingQueue:有界阻塞，FIFO
            * LinkedBlockingQueue：基于链表结构的阻塞队列，FIFO，吞吐量高于ArrayBlockingQueue。Excutors.newFixedThreadPool使用的是这个队列
            * SynchronousQueue：不存储元素的阻塞队列，Executors.newCachedThreadPool使用
